...
public class PowerNodes {
    ...
    protected static long processNextNode(Node aCurrentNode, Node aNextNode, NodeList aConsumers, int side, long aMaxAmps, long aVoltage) {
        if (aCurrentNode.locks[side].isLocked()) {
            aConsumers.getNextNode();
            return 0;
        }
        final PowerNodePath tPath = (PowerNodePath) aCurrentNode.mNodePaths[side];
    ...
    protected static long processNextNodeAbove(Node aCurrentNode, Node aNextNode, NodeList aConsumers, int side, long aMaxAmps, long aVoltage) {
        if (aCurrentNode.locks[side].isLocked()) {
            aConsumers.getNextNode();
            return 0;
        }
    ...
    protected static long processNodeInject(Node aCurrentNode, ConsumerNode aConsumer, int side, long aMaxAmps, long aVoltage) {
        if (aCurrentNode.locks[side].isLocked()) return 0;
        final PowerNodePath tPath = (PowerNodePath) aCurrentNode.mNodePaths[side];
        final PowerNodePath tSelfPath = (PowerNodePath) aCurrentNode.mSelfPath;
        long tVoltLoss = 0;
        if (tSelfPath != null) {
