...
public class Container extends net.minecraft.inventory.Container {
	public ITile mTileEntity;
	public InventoryPlayer mPlayerInventory;
	
	public int mXOffset = 0, mYOffset = 0;
	
	public Container(InventoryPlayer aPlayerInventory, ITile aTileEntityInventory) {
	...
	public Container(InventoryPlayer aPlayerInventory, ITile aTileEntityInventory, int aPlayerInventoryXOffset, int aPlayerInventoryYOffset) {
		this(aPlayerInventory, aTileEntityInventory);
		mXOffset = aPlayerInventoryXOffset;
		mYOffset = aPlayerInventoryYOffset;
	}
	
	protected static ItemStack handleFluidSlotClick(IFluidAccess aFluidAccess, EntityPlayer player, boolean aProcessFullStack, boolean aCanDrain, boolean aCanFill) {
		ItemStack tStackHeld = player.inventory.getItemStack();
		ItemStack tStackSizedOne = copyAmount(1, tStackHeld);
		if (tStackSizedOne == null || tStackHeld.stackSize == 0) return null;
		FluidStack tInputFluid = aFluidAccess.get();
		FluidStack tFluidHeld = getFluidForFilledItem(tStackSizedOne, true);
		...
		if (tInputFluid == null) {
			...
			if (tFluidHeld == null)
				// no fluid to fill
				return null;
			return fillFluid(aFluidAccess, player, tFluidHeld, aProcessFullStack);
		}
		...
		if (tFluidHeld != null && tInputFluid.amount < aFluidAccess.getCapacity()) {
			...
			return drainFluid(aFluidAccess, player, aProcessFullStack);
		} else {
	...
	protected static ItemStack drainFluid(IFluidAccess aFluidAccess, EntityPlayer player, boolean aProcessFullStack) {
		...
		ItemStack tFilledContainer = fillFluidContainer(tTankStack, tStackSizedOne, true, false);
		...
		if (tFilledContainer != null) {
			...
			replaceCursorItemStack(player, tFilledContainer);
		}
	...
	protected static ItemStack fillFluid(IFluidAccess aFluidAccess, EntityPlayer player, FluidStack aFluidHeld, boolean aProcessFullStack) {
		...
		if (tFreeSpace >= aFluidHeld.amount) {
			...
			tStackEmptied = getContainerForFilledItem(tStackSizedOne, false);
		...
		replaceCursorItemStack(player, tStackEmptied);
		return tStackEmptied;
	}
	
	private static void replaceCursorItemStack(EntityPlayer player, ItemStack tStackResult) {
		int tStackResultMaxStackSize = tStackResult.getMaxStackSize();
		while (tStackResult.stackSize > tStackResultMaxStackSize) {
			player.inventory.getItemStack().stackSize -= tStackResultMaxStackSize;
			addItemToPlayerInventory(player, tStackResult.splitStack(tStackResultMaxStackSize));
		}
		if (player.inventory.getItemStack().stackSize == tStackResult.stackSize) {
			// every cell is mutated. it could just stay on the cursor.
			player.inventory.setItemStack(tStackResult);
		} else {
			...
			addItemToPlayerInventory(player, tStackResult);
		}
	}
	...
	/**
	 * Override this Function with something like "return mTileEntity.isUseableByPlayer(player);"
	 */
	@Override
	...
	public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer player) {
		...
		if (aSlotIndex >= 0) {
			if (inventorySlots.get(aSlotIndex) == null || inventorySlots.get(aSlotIndex) instanceof Slot_Holo)
				return null;
		...
		InventoryPlayer aPlayerInventory = player.inventory;
		Slot aSlot;
		...
		if ((aShifthold == 0 || aShifthold == 1) && (aMouseclick == 0 || aMouseclick == 1)) {
			if (aSlotIndex == -999) {
				if (aPlayerInventory.getItemStack() != null) {
					if (aMouseclick == 0) {
						player.dropPlayerItemWithRandomChoice(aPlayerInventory.getItemStack(), true);
						aPlayerInventory.setItemStack(null);
					}
					if (aMouseclick == 1) {
						player.dropPlayerItemWithRandomChoice(aPlayerInventory.getItemStack().splitStack(1), true);
				...
				if (aSlot != null && aSlot.canTakeStack(player)) {
					tTempStack = this.transferStackInSlot(player, aSlotIndex);
					if (tTempStack != null) {
						rStack = copyOrNull(tTempStack);
						if (aSlot.getStack() != null && aSlot.getStack().getItem() == tTempStack.getItem()) {
							slotClick(aSlotIndex, aMouseclick, aShifthold, player);
						}
					}
				}
			} else {
				...
				if (aSlot != null) {
					...
					if (tTempStack == null) {
						...
						if (mouseStack == null) {
							...
							aSlot.onPickupFromSlot(player, aPlayerInventory.getItemStack());
				...
				if (canSwap && aSlot.getHasStack()) {
					...
					if (tTempStack != null && (aSlot.inventory != aPlayerInventory || !aSlot.isItemValid(tTempStack))) {
						if (tTempStackSize > -1) {
							...
							aSlot.putStack(null);
							aSlot.onPickupFromSlot(player, aHoldStack);
						}
					} else {
						aSlot.decrStackSize(aHoldStack.stackSize);
		...
		} else if (aShifthold == 3 && player.capabilities.isCreativeMode && aPlayerInventory.getItemStack() == null && aSlotIndex >= 0) {
			aSlot = (Slot) this.inventorySlots.get(aSlotIndex);
			if (aSlot != null && aSlot.getHasStack()) {
				tTempStack = copyOrNull(aSlot.getStack());
				if (tTempStack != null) {
	...
	@Override
	public ItemStack transferStackInSlot(EntityPlayer player, int aSlotIndex) {
		...
		if (getSlotCount() > 0 && slotObject != null && slotObject.getHasStack() && !(slotObject instanceof Slot_Holo)) {
			ItemStack stackInSlot = slotObject.getStack();
			stack = copyOrNull(stackInSlot);
	 ...
	 */
	@Override
	protected boolean mergeItemStack(ItemStack stack, int aStartIndex, int aSlotCount, boolean reverseOrder) {
		...
		if (stack.isStackable()) {
			while (stack.stackSize > 0 && (!reverseOrder && slotIndex < aSlotCount || reverseOrder && slotIndex >= aStartIndex)) {
				slot      = (Slot) this.inventorySlots.get(slotIndex);
				itemStack = slot.getStack();
				if (!(slot instanceof Slot_Holo) && !(slot instanceof Slot_Output) && slot.isItemValid(stack) && itemStack != null && itemStack.getItem() == stack.getItem() && (!stack.getHasSubtypes() || stack.getItemDamage() == itemStack.getItemDamage()) && ItemStack.areItemStackTagsEqual(stack, itemStack)) {
					int combinedStackSize = itemStack.stackSize + stack.stackSize;
		...
		if (stack.stackSize > 0) {
			...
			while (!reverseOrder && slotIndex < aSlotCount || reverseOrder && slotIndex >= aStartIndex) {
				...
				if (slot.isItemValid(stack) && itemStack == null) {
					int quantityToTransfer = Math.min(stack.stackSize, mTileEntity.getInventoryStackLimit());
					slot.putStack(copyAmount(quantityToTransfer, stack));
					slot.onSlotChanged();
					stack.stackSize -= quantityToTransfer;
					transferredStack = true;
					break;
				}
