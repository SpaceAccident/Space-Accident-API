...
public abstract class MetaGeneratedItem32 extends MetaGeneratedItem {
	...
	public MetaGeneratedItem32(String unLocalName, OrePrefixes... ores) {
		...
		for (int i = 0; i < MAX_COUNT_AUTOGENERATED_ITEMS; i++) {
			...
			if (doesMaterialAllowGeneration(ore, mat)) {
				...
				LanguageManager.addStringLocalization(getUnlocalizedName(tStack) + ".name", getDefaultLocalizationFormat(ore, mat, i));
				LanguageManager.addStringLocalization(getUnlocalizedName(tStack) + ".tooltip", mat.getToolTip());
				OreDictUnifier.registerOre(ore.get(mat), tStack);
			}
		}
	}
	...
	public int[] getRGBa(ItemStack stack) {
		Materials tMaterial = API.sGeneratedMaterials[getDamage(stack) % MAX_MATERIALS];
	 ...
	 * @param meta a Index from [0 - 31999]
	 * @return the Localized Name when default LangFiles are used.
	 */
	public String getDefaultLocalization(OrePrefixes aPrefix, Materials aMaterial, int meta) {
	 ...
	 */
	public String getDefaultLocalizationFormat(OrePrefixes aPrefix, Materials aMaterial, int meta) {
	...
	public final IIconContainer getIconContainer(int meta, Materials aMaterial) {
		OrePrefixes ore = mGeneratedPrefixList[meta / MAX_MATERIALS];
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		int aDamage = stack.getItemDamage();
		if (aDamage < MAX_COUNT_AUTOGENERATED_ITEMS && aDamage >= 0) {
			return Utility.getLocalizedNameForItem(name, aDamage % MAX_MATERIALS);
		}
		return name;
	}
	...
	@Override
	public final IIconContainer getIconContainer(int meta) {
		return API.sGeneratedMaterials[meta % MAX_MATERIALS] == null ? null : getIconContainer(meta, API.sGeneratedMaterials[meta % MAX_MATERIALS]);
	}
	...
	public final IIcon getIconFromDamage(int meta) {
		...
		if (meta < MAX_COUNT_AUTOGENERATED_ITEMS) {
			Materials tMaterial = API.sGeneratedMaterials[meta % MAX_MATERIALS];
			if (tMaterial == null) return null;
			IIconContainer tIcon = getIconContainer(meta, tMaterial);
			if (tIcon != null) {
				return tIcon.getIcon();
			}
			return null;
		}
		return meta - MAX_COUNT_AUTOGENERATED_ITEMS < mIconList.length ? mIconList[meta - MAX_COUNT_AUTOGENERATED_ITEMS][0] : null;
	}
	
	@Override
	public int getItemStackLimit(ItemStack stack) {
		int tDamage = getDamage(stack);
		if (tDamage < MAX_COUNT_AUTOGENERATED_ITEMS && mGeneratedPrefixList[tDamage / MAX_MATERIALS] != null) {
			return Math.min(super.getItemStackLimit(stack), mGeneratedPrefixList[tDamage / MAX_MATERIALS].stackSize);
		}
