		...
		if (sEnglishFile == null) {
			...
			if (!BUFFERMAP.isEmpty()) {
				for (Entry<String, String> tEntry : BUFFERMAP.entrySet()) {
					...
					if (!tProperty.wasRead() && API.sPostloadFinished) sEnglishFile.save();
				}
				BUFFERMAP.clear();
			}
		...
		if (StringUtils.isStringInvalid(rTranslation)) {
			...
			if (StringUtils.isStringInvalid(rTranslation) || tTrimmedKey.equals(rTranslation)) {
				if (aKey.endsWith(".name")) {
					...
					if (StringUtils.isStringInvalid(rTranslation) || trimmedKeyNoName.equals(rTranslation)) {
						return aKey;
					}
				} else {
...
//    	addStringLocalization("Interaction_DESCRIPTION_Index_210", "Grab");
		...
		addStringLocalization("Item_DESCRIPTION_Index_019", "Tier: %s");
	...
	public static void propagateLocalizationServerSide() {
		if (!SpaceAccidentApi.proxy.isServerSide()) return;
		try {
	...
	public static void initLocalization(File mc) {
		GT_LanguageManager.sEnglishFile = new Configuration(new File(new File(mc, "SpaceAccident/lang"), "SpaceAccident.lang"));
		...
		MaterialList.getMaterialsMap().values()
				...
				.forEach(aMaterial -> aMaterial.localName = GT_LanguageManager.addStringLocalization("Material." + aMaterial.name.toLowerCase(), aMaterial.localName));
	}
}
