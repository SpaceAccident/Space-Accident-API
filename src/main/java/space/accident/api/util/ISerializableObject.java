...
public interface ISerializableObject {
	...
	void loadDataFromNBT(NBTBase nbt);
	 ...
	 * @param player the player who is sending this packet to server. null if it's client reading data.
	 */
	// the NBT is an unfortunate piece of tech. everything uses it but its API is not as efficient as could be
	@Nonnull
	ISerializableObject readFromPacket(ByteArrayDataInput aBuf, @Nullable EntityPlayerMP player);
	...
	final class LegacyCoverData implements ISerializableObject {
		...
		public void loadDataFromNBT(NBTBase nbt) {
			mData = nbt instanceof NBTTagInt ? ((NBTTagInt) nbt).func_150287_d() : 0;
		}
